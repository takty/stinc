...
function register( $post_type, $slug, $label_s, $hierarchical = true, $show_ui = true ) {
	$args = array(
		...
		'public'            => true,
		'show_ui'           => $show_ui,
		'rewrite'           => array( 'with_front' => false, 'slug' => "{$post_type}/{$slug}" ),
		'sort'              => true,
...
function register_category( $post_type, $slug, $suffix = 'category', $label_s = false ) {
	$args = array(
		'rewrite'           => array( 'with_front' => false, 'slug' => "$slug/$suffix" ),
		'hierarchical'      => true,
		'public'            => true,
		'show_ui'           => true,
		'sort'              => true,
...
function set_terms( $taxonomy, $slugs_to_labels, $parent_id = 0, $force_rename = false ) {
	foreach ( $slugs_to_labels as $slug => $label ) {
		...
		if ( is_array( $label ) ) {
			if ( false === $term ) {
				$ret = wp_insert_term( $label[0], $taxonomy, array( 'slug' => $slug, 'parent' => $parent_id ) );
				set_terms( $taxonomy, $label[1], $ret['term_id'] );
			} elseif ( $force_rename ) {
				wp_update_term( $term->term_id, $taxonomy, array( 'name' => $label[0] ) );
				set_terms( $taxonomy, $label[1], $term->term_id, $force_rename );
			}
		} else {
			if ( false === $term ) {
				wp_insert_term( $label, $taxonomy, array( 'slug' => $slug, 'parent' => $parent_id ) );
			} elseif ( $force_rename ) {
				wp_update_term( $term->term_id, $taxonomy, array( 'name' => $label ) );
			}
		}
	}
}
...
function set_taxonomy_post_type_specific( $taxonomies, $post_type ) {
	add_action(
		'pre_get_posts',
		function ( $query ) use ( $taxonomies, $post_type ) {
...
function set_taxonomy_default_term( $post_type_s, $taxonomy, $default_term_slug ) {
	...
	foreach( $post_type_s as $post_type ) {
		add_action(
			...
			function ( $post_id, $post ) use ( $post_type_s, $taxonomy, $default_term_slug ) {
...
function set_taxonomy_exclusive( $taxonomy_s ) {
	...
	add_action(
		'set_object_terms',
		function ( $object_id, $terms, $tt_ids, $taxonomy, $append, $old_tt_ids ) use ( $taxonomy_s ) {
			if ( ! in_array( $taxonomy, $taxonomy_s, true ) ) {
				return;
			}
...
function _cb_edited_term( $term_id, $taxonomy ) {
	if ( isset( $_POST[ \st\TMK_NAME_S ] ) ) {
		...
		if ( empty( $val ) ) {
			return delete_term_meta( $term_id, \st\TMK_NAME_S );
		}
...
function get_term_root( $term, $root_id ) {
	...
	while ( 0 !== $cur && $cur !== $root_id ) {
		if ( 0 === $term->parent || $term->parent === $root_id ) {
			break;
		}
...
function count_term_from_posts( $posts, $taxonomy, $term_slug ) {
	$post_sets = array();
	foreach ( $posts as $p ) {
		$terms = wp_get_object_terms( $p->ID, $taxonomy, array( 'fields' => 'slugs' ) );
		foreach ( $terms as $t ) {
			if ( ! isset( $post_sets[ $t ] ) ) {
				$post_sets[ $t ] = array();
			}
			$post_sets[ $t ][ $p->ID ] = 1;
		}
...
function _count_term( $taxonomy, $term, &$post_sets ) {
	...
	if ( ! isset( $post_sets[ $term->slug ] ) ) {
		$post_sets[ $term->slug ] = array();
	}
