...
function adjacent_posts_rel_link_wp_head() {
	...
	if ( 0 < $numpages ) {
		...
		if ( $url_prev ) $prev = '<link rel="prev" href="'. $url_prev . '" />';
		if ( $url_next ) $next = '<link rel="next" href="' . $url_next . '" />';
	}
	if ( is_single() ) {
		if ( empty( $prev ) ) $prev = get_adjacent_post_rel_link( '%title', false, '', true, 'category' );
		if ( empty( $next ) ) $next = get_adjacent_post_rel_link( '%title', false, '', false, 'category' );
	}
	echo $prev;
	echo $next;
}
...
function get_page_break_count( $post ) {
	...
	if ( false === strpos( $content, '<!--nextpage-->' ) ) return 1;
