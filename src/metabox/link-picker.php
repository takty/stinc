...
class LinkPicker {
	...
	public static function enqueue_script( $url_to = false ) {
		if ( is_admin() ) {
			...
			wp_enqueue_script( 'picker-link', $url_to . '/asset/lib/picker-link.min.js', array( 'wplink', 'jquery-ui-autocomplete' ) );
			wp_enqueue_script( self::NS, $url_to . '/asset/link-picker.min.js', array( 'picker-link', 'jquery-ui-sortable' ) );
	...
	public function get_items( $post_id = false ) {
		...
		foreach ( $its as &$it ) {
			...
			$permalink = get_permalink( (int) $it['post_id'] );
			if ( false !== $permalink && $it['url'] !== $permalink ) {
				$it['url'] = $permalink;
			}
		}
		return $its;
	...
	public function get_posts( $post_id = false, $skip_other_than_post = true ) {
		...
		foreach ( $its as $it ) {
			...
			if ( null === $p && $skip_other_than_post ) {
				continue;
			}
			$ps[] = $p;
		}
	...
	public function add_meta_box( $label_s, $screen, $context = 'advanced' ) {
		...
		\add_meta_box( "{$this->_key}_mb", $title, array( $this, '_cb_output_html' ), $screen, $context );
	}
	...
	public function _cb_output_html( $post ) {  // Private
		...
		if ( $this->_max_count ) {
			$its = array_slice( $its, 0, min( $this->_max_count, count( $its ) ) );
		}
		?>
	...
	private function _output_row( $it, $cls ) {
		...
		<div class="<?php echo $cls ?>">
			<div class="<?php echo self::CLS_ITEM_CTRL ?>">
				...
				<label class="widget-control-remove <?php echo self::CLS_DEL_LAB ?>"><span><?php esc_html_e( 'Remove', 'default' ); ?></span><input type="checkbox" class="<?php echo self::CLS_DEL ?>" /></label>
			</div>
			<div class="<?php echo self::CLS_ITEM_CONT ?>">
				...
				<div class="<?php echo self::CLS_ITEM_IR ?>">
					...
					<span>
						...
						<a href="javascript:void(0);" class="button <?php echo self::CLS_SEL ?>"><?php esc_html_e( 'Select', 'default' ); ?></a>
					</span>
				</div>
			</div>
	...
	private function _save_items( $post_id ) {
		$skeys = array( 'title', 'url', 'post_id', 'delete' );
...
function initialize( $key ) {
	return new \st\LinkPicker( $key );
}
function enqueue_script( $url_to = false ) {
	\st\LinkPicker::enqueue_script( $url_to );
}

function get_items( $key, $post_id = false ) {
	return \st\LinkPicker::get_instance( $key )->get_items( $post_id );
}
function get_posts( $key, $post_id = false, $skip_other_than_post = true ) {
	return \st\LinkPicker::get_instance( $key )->get_posts( $post_id, $skip_other_than_post );
}
function set_internal_only( $key, $enabled ) {
	return \st\LinkPicker::get_instance( $key )->set_internal_only( $enabled );
}
function set_max_count( $key, $count ) {
	return \st\LinkPicker::get_instance( $key )->set_max_count( $count );
}
function set_link_target_allowed( $key, $enabled ) {
	return \st\LinkPicker::get_instance( $key )->set_link_target_allowed( $enabled );
}
function set_post_type( $key, $post_type ) {
	return \st\LinkPicker::get_instance( $key )->set_post_type( $post_type );
}

function add_meta_box( $key, $label_s, $screen, $context = 'advanced', $opts = array() ) {
	if ( isset( $opts['is_internal_only'] ) ) {
		set_internal_only( $key, $opts['is_internal_only'] );
	}
	if ( isset( $opts['max_count'] ) ) {
		set_max_count( $key, $opts['max_count'] );
	}
	if ( isset( $opts['is_link_target_allowed'] ) ) {
		set_link_target_allowed( $key, $opts['is_link_target_allowed'] );
	}
	if ( isset( $opts['post_type'] ) ) {
		set_post_type( $key, $opts['post_type'] );
	}
	\st\LinkPicker::get_instance( $key )->add_meta_box( $label_s, $screen, $context );
}
function save_meta_box( $post_id, $key ) {
	\st\LinkPicker::get_instance( $key )->save_meta_box( $post_id );
}
