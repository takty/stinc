...
function disable_generator_output() {
	...
	foreach ( $as as $a ) remove_action( $a, 'the_generator' );
}

function disable_version_output() {
	add_action( 'wp_default_scripts', function ( $inst ) { $inst->default_version = ''; } );
	add_action( 'wp_default_styles', function ( $inst ) { $inst->default_version = ''; } );
...
function _cb_loader_src_ver( $src ) {
	if ( strpos( $src, 'ver=' ) !== false ) return remove_query_arg( 'ver', $src );
	return $src;
}
