...
function get_post_title( $short = 8, $long = 32, $mode = 'segment_small', $filter = 'esc_html' ) {
	...
	$option = '';
	if ( $long <= $len ) {
		$option = ' data-length="long"';
	}
	if ( $len <= $short ) {
		$option = ' data-length="short"';
	}
...
function get_term_list( $taxonomy, $before = '', $sep = '', $after = '', $add_link = true, $args = array() ) {
...
function get_the_term_list( $post_id, $taxonomy, $before = '', $sep = '', $after = '', $add_link = true, $args = array() ) {
	...
	$singular = isset( $args['singular'] ) ? $args['singular'] : false;
	if ( isset( $args['is_root_inserted'] ) && ( true === $args['is_root_inserted'] ) ) {
		$ts = _insert_root( $ts );
	}
	$filter = isset( $args['filter'] ) ? $args['filter'] : 'esc_html';
...
function create_term_list( $terms, $taxonomy, $before, $sep, $after, $add_link, $current_term = false, $singular = false, $filter = 'esc_html' ) {
	...
	foreach ( $terms as $t ) {
		...
		if ( $current_term && $current_term->term_id === $t->term_id ) {
			$cs[] = 'current';
		}
		$cs_str = implode( ' ', $cs );
...
function get_term_names( $taxonomy, $singular = false, $lang = false, $args = array() ) {
	$ts = empty( $args ) ? get_terms( $taxonomy ) : get_terms( $taxonomy, $args );
	if ( is_wp_error( $ts ) ) {
		return $ts;
	}
